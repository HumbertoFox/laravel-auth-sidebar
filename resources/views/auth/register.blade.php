    ...
    <div class="w-full flex flex-col justify-center gap-6 2xl:w-2/4">
        ...
        <form action="{{ route('register.post') }}" method="POST" class="w-full max-w-xs flex flex-col gap-6 mx-auto">
            ...
            <div class="flex flex-col gap-4">
                <x-ui.input label="Name" name="name" type="text" placeholder="Full name" :value="old('name')"
                ...
                <x-ui.input label="Password" name="password" type="password" placeholder="Password" :error="$errors->first('password')"
                    required />
                <x-ui.input label="Confirm your password." name="password_confirmation" type="password"
                    placeholder="Confirm your password." :error="$errors->first('password_confirmation')" required />
            </div>

            <x-ui.button type="submit" class="w-full">Create an account</x-ui.button>
        </form>
